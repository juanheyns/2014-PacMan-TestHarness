...
namespace PacManDuel.Models
{
    class Game
    {
        ...
        {
            var gamePlayDirectoryPath = System.Environment.CurrentDirectory + System.IO.Path.PathSeparator + folderPath;
            Directory.CreateDirectory(gamePlayDirectoryPath);
            var outputFilePath = gamePlayDirectoryPath + System.IO.Path.PathSeparator + Properties.Settings.Default.SettingGamePlayFile;
            _maze.WriteMaze(outputFilePath);
            Player winner = null;
            var gameOutcome = Enums.GameOutcome.ProceedToNextRound;
            Directory.CreateDirectory(folderPath);
            Directory.CreateDirectory(folderPath + System.IO.Path.PathSeparator + Properties.Settings.Default.SettingReplayFolder);
            var logFile = new StreamWriter(folderPath + System.IO.Path.PathSeparator + Properties.Settings.Default.SettingMatchLogFileName);
            ...
            while (gameOutcome == Enums.GameOutcome.ProceedToNextRound)
            {
                ...
                var mazeFromPlayer = _currentPlayer.GetMove(_maze, gamePlayDirectoryPath + System.IO.Path.PathSeparator + Properties.Settings.Default.SettingGamePlayFile, logFile);
                if (mazeFromPlayer != null)
                {
                    ...
                    _maze.WriteMaze(gamePlayDirectoryPath + System.IO.Path.PathSeparator + Properties.Settings.Default.SettingGamePlayFile);
            ...
            logFile.Close();
            var replayMatchOutcome = new StreamWriter(folderPath + System.IO.Path.PathSeparator + "replay" + System.IO.Path.PathSeparator + "matchinfo.out");
        ...
        {
            var replayFile =
                new StreamWriter(folderPath + System.IO.Path.PathSeparator + Properties.Settings.Default.SettingReplayFolder + System.IO.Path.PathSeparator + "iteration" +
